...
+ fast-forwardマージを無効化して，必ずマージコミットを作成するオプションは何ですか．
...
+ ローカルブランチとリモート追跡ブランチの対応関係を表示するコマンドを記述してください．
+ リモート追跡ブランチの一覧を表示するコマンドを記述してください．
+ リモート追跡ブランチをフェッチするコマンドを記述してください．
+ リモート追跡ブランチ`origin/main`とローカルブランチ`main`の差分確認を行うコマンドを記述してください．
  - リモートにあってローカルにないコミットを出力
  - ローカルにあってリモートにないコミットを出力
+ フェッチしたリモート追跡ブランチの変更をローカルブランチに反映するコマンドを記述してください．
+ リモート追跡ブランチのフェッチとローカルブランチへの変更反映を一括で行うコマンドは何ですか．
+ リモート追跡ブランチのコピーをローカルブランチとして作成し，upstreamの設定も一括で行うコマンドは何ですか．
